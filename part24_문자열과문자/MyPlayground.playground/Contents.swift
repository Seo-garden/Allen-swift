...
//MARK: - 206강
//앞뒤로 문자열 수정이 가능하다. 문자열 중간중간에 있는 것은 수정할 수 없다.
var userEmail = " sgrdn@github.com "

var trimmedString = userEmail.trimmingCharacters(in: [" ", " "])     //공백뿐만 아니라, 문자도 가능하다. 지금의 예시는 앞뒤에
print(trimmedString)        //처음과 마지막에 있는 공백 제거

//중간에 있는 특수문자 제거 원리 : 문자열을 배열로 만들어서 제거 후 다시 문자열로 반환
var nameSteve = "S t e v e +"
//하나만 가능하지 않고, 여러개 삭제도 가능하다(여러개를 삭제 할 경우 배열에 담아야 한다.)
var removedName = name.components(separatedBy: [" ", "+"]).joined()     //공백 문자를 제거하고, 배열로 반환(.joined()) 자주 사용하게 된다.
print(removedName)

/**=========================================
 [CharacterSet] 유니코드 기준 절대 외울필요가 없다. 찾아서 쓰면 된다.
 - .symbols                // 기호
 - .alphanumerics          // 문자 + 숫자
